...
local function notify(msg, level)
  ...
  vim.notify(msg, level, { title = "Flutter Guides" })
end
...
local function highlight(name, opts)
  ...
  for k, v in pairs(opts) do
    table.insert(hls, string.format("%s=%s", k, v))
  end
  local hl_cmd = string.format("highlight! %s %s", name, table.concat(hls, " "))
  vim.cmd(hl_cmd)
  augroup(name .. "Refresh", {
    {
      events = { "ColorScheme" },
      targets = { "*" },
      command = hl_cmd,
...
function M.setup()
  local color = vim.fn.synIDattr(vim.fn.hlID("Normal"), "fg")
  if color and color ~= "" then
    highlight(hl_group, { guifg = color })
  end
end
